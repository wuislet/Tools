...
public class AddFileHeadComment : UnityEditor.AssetModificationProcessor
{
    ...
    public static void OnWillCreateAsset(string newFileMeta)
    {
        ...
        if (fileExt != ".cs" && fileExt != ".js" && fileExt != ".boo" && fileExt != ".lua")
        {
            return;
        }
        //注意，Application.datapath会根据使用平台不同而不同  
        int index = Application.dataPath.LastIndexOf("Assets");
        string realPath = Application.dataPath.Substring(0, index) + newFilePath;
        ...
        scriptContent = scriptContent.Replace("#AUTHOR#", "WuIslet");
        scriptContent = scriptContent.Replace("#VERSION#", "1.0");
        scriptContent = scriptContent.Replace("#UNITYVERSION#", Application.unityVersion);
        scriptContent = scriptContent.Replace("#DATE#", System.DateTime.Now.ToString("yyyy-MM-dd"));
        scriptContent = scriptContent.Replace("#FILEEXTENSION#", fileExt);
